...
export function buildReviewPrompt(input: ReviewPromptInput): string {
  return [
    ...
    NO_RECURSION_INSTRUCTION,
    "Do not run commands, invoke tools, or browse. Review only the supplied repository context and diff.",
    ...
    '      "body": "Why this is a problem and what should change",',
    '      "file": "optional repository-relative path",',
    '      "line": 1',
    "    }",
    "  ]",
    "}",
    'Use status "clean" only when findings is empty. Use status "findings" when one or more findings exist.',
...
export function buildVerificationPrompt(input: VerificationPromptInput): string {
  return [
    ...
    NO_RECURSION_INSTRUCTION,
    "Do not run commands, invoke tools, or browse. Assess only the supplied verification evidence.",
    "",
